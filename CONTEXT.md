...
**分钟桶 (Minute Bucket)**:
60 秒聚合单元。metric 定义当天的桶网格（t0 起每 60s 一桶），log/trace 是离散事件流，按各自精确时间戳落桶计数聚合；空桶为零，不做插值。
_Avoid_: 采集间隔（log/trace 本身不等间隔，"一分钟"只是聚合粒度）

**日志模板 (Log Template)**:
Drain 对日志消息聚类得到的模式（变量已被掩码替换，如 `<:NUM:>`），是 Ada-MGAD 日志特征的词元。
_Avoid_: 日志原文、日志模式

**模板词表 (Template Vocabulary)**:
预处理期固定下来的模板集合，其大小决定 `log_len`。Nezha 数据集的词表由训练天与测试天日志合并挖掘而成（有意为之，见 ADR-0001），解读指标时须知词表含测试天信息。
_Avoid_: 词表冻结（本项目未采用）
