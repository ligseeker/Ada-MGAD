...
from util.SN.constant import SN_SERVICE2NID, SN_NUM_NODES
...
class Process:
    def __init__(self, args):
        ...
        if not os.path.exists(bounds_path):
            raise FileNotFoundError(f"{bounds_path} not found; run util/SN/pre_SN.py first")
        ...
        if os.path.exists(dataset_file):
            try:
                ...
                logging.warning(
                    "Failed to read cached SN dataset from %s: %s. Rebuilding from raw data.",
                    self.dataset_path,
                    exc,
                )
            ...
            self._rebuild_from_raw()
        if not self.dataset:
            logging.warning("Cached SN dataset is empty, rebuilding from raw data.")
            self._rebuild_from_raw()
    ...
    def _rebuild_from_raw(self):
        self.load_raw()
        logging.info("Transform data into time windows (within each experiment)")
        self.dataset = self._transform()
        self.save_data()

    def load_raw(self):
        if not os.path.exists(self.rawdata_path):
            raise FileNotFoundError(f"Find no SN preprocessed data at {self.rawdata_path}")
        logging.info("LOADing SN data ...")
        ...
        # SN is fully labeled: the unknown class is always 0
    ...
    def _load_logs(self, T_total):
        ...
        if len(log):
            host_idx = log['Hostname'].map(SN_SERVICE2NID)
        ...
        logging.info(f"Log data: {len(log)} rows, present seconds {int(present.sum())}/{T_total}")
    ...
    def _load_traces(self, T_total):
        ...
        for bound in self.bounds:
            ...
            if not len(seg):
                continue
            src = seg['cmbd_id'].map(SN_SERVICE2NID)
            dst = seg['fatherpod'].map(SN_SERVICE2NID)
            k = seg['stats'].map(stat2idx)
            valid = src.notna() & dst.notna() & k.notna()
            seg = seg[valid]
            T_exp = bound['end'] - bound['start']
            trace_a = np.zeros((self.num_node, self.num_node, num_stats, T_exp))
            np.add.at(
                trace_a,
    ...
    def _transform(self):
        ...
        for exp_id, bound in enumerate(self.bounds):
            ...
            logging.info(f"experiment {exp_id} ({bound['name']}): windows so far {num}")
    ...
    def _build_split(self):
        ...
        logging.info(
            "SN split: held-out fault experiment %d (%s) | train=%d (abnormal %.2f%%) test=%d (abnormal %.2f%%)",
