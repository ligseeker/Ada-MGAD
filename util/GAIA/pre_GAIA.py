...
def configure_paths(raw_path, save_path):
    global Raw_Path, Save_Path, BUSINESS_DIR, TRACE_DIR, METRIC_DIR, LABEL_DIR
...
configure_paths(Raw_Path, Save_Path)


def parse_cli_args():
    parser = argparse.ArgumentParser(description='Preprocess GAIA raw data for the anonymous Ada-MGAD artifact.')
    parser.add_argument(
        '--raw_path',
        default=Raw_Path,
        help='Path to the GAIA MicroSS raw dataset root.',
    )
    parser.add_argument(
        '--save_path',
        default=Save_Path,
        help='Directory for the preprocessed GAIA output files.',
    )
    parser.add_argument(
        '--step',
        default='',
        help='Comma-separated step ids to run. Empty means run all steps.',
    )
    parser.add_argument(
        '--force',
        action='store_true',
        help='Re-run selected preprocessing steps even if outputs already exist.',
    )
...
def deal_metric(metric_path, save_path):
    ...
    service_col_map = defaultdict(dict) 
    ...
    base_feat_low_var_count = defaultdict(int)  
    base_feat_total_count = defaultdict(int)    
    col_to_base = {}
...
def main():
    ...
    configure_paths(cli_args.raw_path, cli_args.save_path)
    os.makedirs(Save_Path, exist_ok=True)
    ...
    if cli_args.step:
        steps = [step.strip() for step in cli_args.step.split(',') if step.strip()]
        run_steps = set(int(step) for step in steps)
